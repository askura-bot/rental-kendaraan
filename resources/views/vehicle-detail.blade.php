<x-public-layout>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="p-4 border-t border-gray-200">
                                <div class="grid grid-cols-4 sm:grid-cols-6 gap-2">
                                    ...
                                    @foreach($vehicle->images as $image)
                                        <div
                                            class="relative h-20 rounded cursor-pointer border-2 border-gray-300 hover:border-blue-500 transition"
                    ...
                    <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                            ...
                            <div class="border-l-4 border-blue-500 pl-4">
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ $vehicle->brand }}</p>
                            </div>

                            <!-- Year -->
                            <div class="border-l-4 border-green-500 pl-4">
                                <p class="text-sm text-gray-600 font-medium">{{ __('Year') }}</p>
                                <p class="text-lg font-bold text-gray-900">{{ $vehicle->year }}</p>
                            </div>

                            <!-- Category -->
                            <div class="border-l-4 border-purple-500 pl-4">
                                <p class="text-sm text-gray-600 font-medium">{{ __('Category') }}</p>
                                <p class="text-lg font-bold text-gray-900">{{ $vehicle->category->name }}</p>
                            </div>

                            <!-- Engine -->
                            <div class="border-l-4 border-red-500 pl-4">
                                <p class="text-sm text-gray-600 font-medium">{{ __('Engine (CC)') }}</p>
                                <p class="text-lg font-bold text-gray-900">{{ number_format($vehicle->cc) }}</p>
                            </div>

                            <!-- Capacity -->
                            <div class="border-l-4 border-yellow-500 pl-4">
                                <p class="text-sm text-gray-600 font-medium">{{ __('Passenger Capacity') }}</p>
                                <p class="text-lg font-bold text-gray-900">{{ $vehicle->capacity }} {{ __('seats') }}</p>
                            </div>

                            <!-- Transmission -->
                            <div class="border-l-4 border-indigo-500 pl-4">
                                <p class="text-sm text-gray-600 font-medium">{{ __('Transmission') }}</p>
                                <p class="text-lg font-bold text-gray-900">{{ ucfirst($vehicle->transmission) }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-6 space-y-6">
                        <!-- Info Card -->
                        @php
                            $preBookingInfo = \App\Models\Setting::getValue('pre_booking_information', "✓ Have your ID ready\n✓ Bring a valid driving license\n✓ Prepare security deposit\n✓ Check terms & conditions");
                        @endphp
                        @if($preBookingInfo)
                            <div class="bg-amber-50 border border-amber-200 rounded-lg p-4">
                                <h3 class="font-bold text-amber-900 mb-2">⚠️ {{ __('Before Booking') }}</h3>
                                <div class="text-sm text-amber-800 space-y-1">
                                    {!! nl2br(e($preBookingInfo)) !!}
                                </div>
                            </div>
                        @endif
                        ...
                        <!-- Pricing Card -->
                        <div class="bg-white rounded-lg shadow-lg p-6">
                            <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('Rental Price') }}</h2>

                            <!-- 12 Hour Price -->
                            <div class="mb-6 pb-6 border-b border-gray-200">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-gray-600 font-medium">{{ __('12 Hours') }}</span>
                                    <span class="text-2xl font-bold text-gray-900">
                                        Rp {{ number_format($vehicle->price_12h) }}
                                    </span>
                                </div>
                                <p class="text-xs text-gray-500">{{ __('per rental') }}</p>
                            </div>

                            <!-- 24 Hour Price -->
                            <div class="mb-6 pb-6 border-b border-gray-200">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-gray-600 font-medium">{{ __('24 Hours') }}</span>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        @foreach($relatedVehicles as $related)
                            <a href="{{ route('vehicle-detail', $related->id) }}" class="group">
                                <div class="bg-white rounded-lg shadow-md hover:shadow-xl:shadow-lg transition overflow-hidden h-full flex flex-col">
                                    <!-- Image -->
                                    ...
                                    <div class="p-3 flex-1 flex flex-col">
                                        <h3 class="text-sm font-bold text-gray-900 group-hover:text-blue-600:text-blue-400 transition">
                                            {{ $related->name }}
                                        </h3>
    ...
    <div id="imageModal" class="hidden fixed inset-0 bg-black/75 z-50 items-center justify-center p-4" onclick="closeImageModal()">
        <div class="relative max-w-4xl w-full bg-white rounded-lg" onclick="event.stopPropagation()">
            <button type="button" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700:text-gray-200 z-10" onclick="closeImageModal()">
