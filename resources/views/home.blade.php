            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
                ...
                <div class="relative">
                    <div class="grid grid-cols-2 gap-4">
                        @php $aboutImages = $vehicles->take(2); @endphp
                        @foreach($aboutImages as $img)
                            @if($img->thumbnail)
                                <div class="rounded-2xl overflow-hidden shadow-lg {{ $loop->first ? 'aspect-[4/5]' : 'aspect-[4/5] mt-8' }}">
                                    <img src="{{ asset('storage/' . $img->thumbnail) }}" alt="{{ $img->name }}" class="w-full h-full object-cover">
                                </div>
                            @else
                                <div class="rounded-2xl bg-gray-100 {{ $loop->first ? 'aspect-[4/5]' : 'aspect-[4/5] mt-8' }} flex items-center justify-center">
                                    <svg class="w-16 h-16 text-gray-300" fill="currentColor" viewBox="0 0 24 24"><path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z"/></svg>
                                </div>
                            @endif
                        @endforeach
                    </div>
    ...
    <section class="py-12 sm:py-20 bg-primary-50/50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @php
                    $testimonials = [
                        ['name' => 'Loren Tamvien', 'role' => __('Business Owner'), 'text' => __('Great service! The car was clean, well-maintained, and delivered on time. Will definitely rent again.')],
                        ['name' => 'Samon Ngoment', 'role' => __('Frequent Traveler'), 'text' => __('Best rental experience I\'ve had. Very professional staff and competitive pricing. Highly recommended!')],
                        ['name' => 'Dewi Sartika', 'role' => __('Event Planner'), 'text' => __('Reliable and trustworthy. I rent from DriveEase for all my events. Always excellent vehicles and service.')],
                    ];
                @endphp
